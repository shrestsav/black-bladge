<template>
  <div class="card">
    <div class="card-header">
      <div class="row">
        <div class="col">
          <h3 class="mb-0">مزودو البيانات</h3>
        </div>
        <div class="col-auto">
    ...
    <div class="card-body" v-if="module.display">
      <div class="accordion" id="dataProvidersArabic">
        <div class="card" v-for="(data_provider,index) in data_providers" :key="index">
          <div class="col-md-12" v-if="editdata_providers.status && editdata_providers.index==index">
            ...
            <div class="form-group">
              <textarea class="form-control" rows="8" placeholder="CONTENT GOES HERE" v-model="data_provider.content"></textarea>
            </div>
          ...
          <template v-else>
            ...
            <div :id="'collapse-'+index" class="collapse" :aria-labelledby="'heading-'+index" data-parent="#dataProvidersArabic">
              <div class="card-body">
                <p>{{data_provider.content}}</p>
                <div class="text-right">
                  <button type="button" class="btn btn-info btn-sm" @click="edit(index)">Edit</button>
                  <button type="button" class="btn btn-danger btn-sm" @click="deletedata_provider(index)">Delete</button>
                </div>
              </div>
            </div>
          </template>
      ...
      <div class="row" v-if="adddata_providers">
        <div class="col-md-12" >
          ...
          <div class="form-group">
            <textarea class="form-control" rows="8" placeholder="CONTENT GOES HERE" v-model="newdata_providers.content"></textarea>
          </div>
        </div>
      </div>
      <div class="text-right">
        <button type="button" class="btn btn-primary btn-sm" @click="adddata_provider" v-if="!adddata_providers">Add More</button>
        <button type="button" class="btn btn-info btn-sm" @click="savedata_provider" v-if="adddata_providers">Save</button>
        <button type="button" class="btn btn-danger btn-sm" @click="cancelAdd" v-if="adddata_providers">Cancel</button>
      </div>
    </div>
  </div>
</template>
    ...
    methods:{
      ...
      save(){
        var data = {
          ...
          data_providers: {
            en: this.data_providersEnglish,
            ar: this.data_providers
          }
        }

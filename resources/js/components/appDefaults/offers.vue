        ...
        <div class="table-responsive" v-if="module.display">
            <table class="table align-items-center table-flush">
                ...
                <tbody>
                        ...
                        <td>
                            ...
                            <div class="custom-file" style="width: 274px; ">
                                <input
                                    ...
                                    lang="en"
                                    ref="offerFile"
                                    v-on:change="offerFileUpload()"
                                    :class="{'not-validated':errors.offer_image}"
                                />
                                <label class="custom-file-label">Offer Image</label>
                                <div
                                    class="invalid-feedback"
                                    style="display: block;"
                                    v-if="errors.offer_image"
                                >{{errors.offer_image[0]}}</div>
                            </div>
                        </td>
                        <td>
                            <div class="form-group">
                                <input
                                    ...
                                    :class="{'not-validated':errors.offer_name}"
                                    type="text"
                                    class="form-control"
                                ...
                                <div
                                    class="invalid-feedback"
                                    style="display: block;"
                                    v-if="errors.offer_name"
                                >{{errors.offer_name[0]}}</div>
                            </div>
                        </td>
                        <td>
                            <div class="form-group">
                                <textarea
                                    ...
                                    :class="{'not-validated':errors.offer_description}"
                                    class="form-control"
                                    rows="8"
                                    placeholder="BRIEF DESCRIPTION OF OFFER"
                                    style="height: 250px;"
                                ></textarea>
                                <div
                                    class="invalid-feedback"
                                    style="display: block;"
                                    v-if="errors.offer_description"
                                >{{errors.offer_description[0]}}</div>
                            </div>
                        </td>
                        <td>
                            <select class="form-control" v-model="offer.offer_display_type">
                    ...
                    <!-- For Old Order List -->
                    <tr v-for="(item,key) in offers" :key="key">
                        <td>{{key+1}}</td>
                        <td>
						...
						<td>
							<select
                                class="form-control"
                                v-model="offer.display_type"
                                v-if="editExistingOrder(item['id'])"
                            >
                                <option value="1">Signup</option>
                                <option value="2">Homescreen</option>
                            </select>
							<template v-else>{{item.display_type_str}}</template>
                        </td>
                        <td>
                            <select
                                class="form-control"
...
export default {
    ...
    methods: {
        ...
        offerFileUpload() {
            ...
            this.offer.image_src = URL.createObjectURL(this.offer.offer_image);
        },
        editOfferImage(e) {
            this.offer.offer_image = e.target.files[0];
            this.offer = Object.assign({}, this.offer, {
                image_src: URL.createObjectURL(this.offer.offer_image)
            });
        },
        saveOffer() {
        ...
        saveEditedOffer() {
            ...
            axios
                .post("/offers/edit/" + this.offer.id, offerForm)
                .then(response => {
                    this.$store.dispatch("getOffers");
                    this.offer = {};
                    this.modifyOrder.id = "";
        ...
        changeOfferStatus(key) {
            this.$swal({
                ...
                if (result.value) {
                    axios
                        .post("/changeOfferStatus/", this.offers[key])
                        .then(response => {
                            this.$store.dispatch("getOffers");
                            showNotify("success", response.data);
                        })

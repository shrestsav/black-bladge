<template>
  <div class="card">
    <div class="card-header">
      <div class="row">
        <div class="col">
          <h3 class="mb-0">المعلومات المحلية</h3>
        </div>
        <div class="col-auto">
    ...
    <div class="card-body" v-if="module.display">
      <div class="accordion" id="localInformationsArabic">
        <div class="card" v-for="(local_information,index) in local_informations" :key="index">
          <div class="col-md-12" v-if="editlocal_informations.status && editlocal_informations.index==index">
            ...
            <div class="form-group">
              <textarea class="form-control" rows="8" placeholder="CONTENT GOES HERE" v-model="local_information.content"></textarea>
            </div>
          ...
          <template v-else>
            ...
            <div :id="'collapse-'+index" class="collapse" :aria-labelledby="'heading-'+index" data-parent="#localInformationsArabic">
              <div class="card-body">
                <p>{{local_information.content}}</p>
                <div class="text-right">
                  <button type="button" class="btn btn-info btn-sm" @click="edit(index)">Edit</button>
                  <button type="button" class="btn btn-danger btn-sm" @click="deletelocal_information(index)">Delete</button>
                </div>
              </div>
            </div>
          </template>
      ...
      <div class="row" v-if="addlocal_informations">
        <div class="col-md-12" >
          ...
          <div class="form-group">
            <textarea class="form-control" rows="8" placeholder="CONTENT GOES HERE" v-model="newlocal_informations.content"></textarea>
          </div>
        </div>
      </div>
      <div class="text-right">
        <button type="button" class="btn btn-primary btn-sm" @click="addlocal_information" v-if="!addlocal_informations">Add More</button>
        <button type="button" class="btn btn-info btn-sm" @click="savelocal_information" v-if="addlocal_informations">Save</button>
        <button type="button" class="btn btn-danger btn-sm" @click="cancelAdd" v-if="addlocal_informations">Cancel</button>
      </div>
    </div>
  </div>
</template>
    ...
    methods:{
      ...
      save(){
        var data = {
          ...
          local_informations: {
            en: this.local_informationsEnglish,
            ar: this.local_informations
          }
        }
